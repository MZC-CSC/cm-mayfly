...
- [문제 해결 — 바꿨는데 반영되지 않을 때](#문제-해결--바꿨는데-반영되지-않을-때)
  - [무엇을 바꿨나 → 어떤 명령을 쓰나](#무엇을-바꿨나--어떤-명령을-쓰나)
  - [재시작으로는 반영되지 않습니다](#재시작으로는-반영되지-않습니다)
  - [반영됐는지 확인하는 방법](#반영됐는지-확인하는-방법)
  - [자주 겪는 증상](#자주-겪는-증상)
...
## 문제 해결 — 바꿨는데 반영되지 않을 때

명령은 성공했다고 하는데 실행 중인 컨테이너는 예전 그대로인 경우가 있습니다. **무엇을 바꿨는지에 따라 써야 할 명령이 다르기** 때문입니다.
...
| **컨테이너가 마운트해 읽는 설정 파일** | `conf/` 하위 yaml 등 | `mayfly infra stop -s <서비스>` → `mayfly infra run -s <서비스>` | 파일은 볼륨이라 바로 보이지만 대개 기동 시점에만 읽습니다 |

### 재시작으로는 반영되지 않습니다

**환경변수는 컨테이너를 만들 때 고정됩니다.** 컨테이너를 껐다 켜는 것으로는 나중에 추가한 환경변수가 들어갈 자리가 없습니다.

값을 바꾸려면 **컨테이너를 다시 만들어야** 하고, 그 일을 하는 것이 `mayfly infra run`입니다. 설정이 바뀐 컨테이너만 골라 다시 만들고 나머지는 그대로 두므로, 전체를 내렸다 올릴 필요가 없습니다.

```bash
# 설정을 바꾼 서비스만 다시 만들기
$ ./mayfly infra run -s airflow-server -d
```

> **재생성이라고 데이터가 사라지지는 않습니다.** 대부분의 서비스는 상태를 컨테이너 밖(별도 DB 컨테이너, `./conf/docker/data` 호스트 볼륨)에 두기 때문입니다. 다만 여럿이 함께 쓰는 환경이라면 진행 중인 작업이 없는지 먼저 확인하세요.

### 반영됐는지 확인하는 방법

명령이 성공했다는 메시지는 근거가 되지 않습니다. 컨테이너에 실제로 들어갔는지 확인하세요.

```bash
# 서비스 상태와 기동 시각 (방금 다시 만들었다면 기동 시각이 짧습니다)
$ ./mayfly infra info

# 갱신이 필요한 이미지가 있는지 — 확인 프롬프트에서 취소하면 아무것도 내려받지 않습니다
$ ./mayfly infra update

# 환경변수가 실제로 들어갔는지 (비밀번호는 가려서 봅니다)
$ docker inspect <컨테이너> --format '{{range .Config.Env}}{{println .}}{{end}}' \
    | grep -i <찾는키> | sed 's/:[^:@]*@/:***@/'
```

`mayfly infra update`는 **확인 프롬프트가 이미지 내려받기보다 앞에** 있습니다. 취소하면 이미지와 컨테이너에 아무 변화가 없으므로, 현재 상태를 살펴보는 용도로 안전하게 쓸 수 있습니다.

### 자주 겪는 증상

**워크플로우가 대기 상태에서 진행되지 않습니다**

`docker-compose.yaml`에 추가된 환경변수가 실행 중인 컨테이너에 적용되지 않아 생길 수 있습니다. 예를 들어 워크플로우 엔진은 여러 개의 프로세스가 각기 다른 연결 설정을 쓰는데, 그중 일부만 값을 받지 못하면 **그 프로세스만 죽고 나머지는 살아 있습니다.** 컨테이너는 healthy로 보이고 웹 화면도 정상이라, 어느 로그에도 원인이 남지 않습니다.

→ `mayfly infra run -s <서비스>`로 다시 만든 뒤, 위 `docker inspect`로 값이 들어갔는지 확인하세요.

> 기본값을 그대로 쓰는 환경에서는 그 설정이 없어도 동작하므로 증상이 나타나지 않습니다. **지금 잘 돌아간다는 것이 설정이 맞다는 뜻은 아닙니다.**

**`update`가 "모두 최신"이라는데 새 이미지가 있습니다**

예전 동작입니다. 태그 *이름*만 비교해서 `edge`·`latest`처럼 이름이 바뀌지 않는 태그는 항상 최신으로 표시됐습니다. 현재는 태그가 가리키는 내용까지 비교해 `◆`로 구분합니다. 낮은 버전을 쓰고 있다면 cm-mayfly부터 최신으로 올리세요.
